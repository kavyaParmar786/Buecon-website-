/* ═══════════════════════════════════════════
   ...
   Uses real catalogue images from assets/images/
   ═══════════════════════════════════════════ */
...
function renderProducts() {
  ...
  const cardsHTML = BUECON.products.map(p => {
    /* Use image_url from Supabase if set, else fall back to local asset */
    const imgSrc = p.image_url || `assets/images/${p.id}.jpg`;
    const hasBanner = p.image_url || true; // we always have local
    ...
    <div class="product-card" data-product-id="${p.id}" data-style="${(p.style||[]).join(' ')}">
      ...
      <div class="card-view-toggle">
        <button class="toggle-btn active" data-view="image" data-product="${p.id}"
          onclick="ProductViewer.setView('${p.id}','image',this)">
          <span>◫</span> Image
        </button>
        <button class="toggle-btn" data-view="3d" data-product="${p.id}"
      ...
      <div class="card-info">
        ...
        <div class="card-footer">
          <span class="card-explore">Explore <span class="card-explore-arrow">→</span></span>
        </div>
      </div>
